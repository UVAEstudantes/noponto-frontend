...
const Home = () => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: cores.fundoApp }}>
      ...
      <MapControls lateralidade={preferenciaLateralidade} location={location} mapRef={mapRef} onOpenLines={() => setContainerAberto(true)} onOpenSearch={() => setBuscaAberta(true)} onOpenFilter={() => setFiltroAberto(true)} disabled={filtroAberto || Boolean(paradaSelecionada) || containerAberto} />
      <LinhasContainer linhasSelecionadas={linhasSelecionadasFiltradas} aoRemoverLinha={removerLinha} aoToggleAtiva={toggleAtiva} aoToggleSentido={toggleSentido} aoToggleParadas={toggleParadas} aoAtualizarCor={atualizarCorLinha} sentidosPorLinha={sentidosPorLinha} aberto={containerAberto} aoToggleAberto={() => setContainerAberto((p) => !p)} modalAtivo={modalSelecionadoNome} mostrarBotaoToggle={false} />
